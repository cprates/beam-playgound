...
import (
	...
	"github.com/apache/beam/sdks/go/pkg/beam/io/elasticsearch"
...
var (
	wordRE  = regexp.MustCompile(`[a-zA-Z]+('[a-z0-9])?`)
...
func formatFn(w string, c int) Summary {
	return Summary{Word: w, Count: c}
}
...
func main() {
	...
	esClient, err := es.NewClient(esConf)
	if err != nil {
		log.Fatal(err)
	}
	...
	elasticsearch.WriteBulk(s, elasticsearch.NewIndex("test-index", esClient), formatted)
